...
namespace Viso
{
  ...
  Matcher::~Matcher()
  {
    ...
    if (I1p_dv)
      _mm_free(I1p_dv);
    if (I1p_du_full)
      _mm_free(I1p_du_full);
    if (I1p_dv_full)
      _mm_free(I1p_dv_full);
    if (m2p1)
      _mm_free(m2p1);
    if (m2p2)
      _mm_free(m2p2);
    ...
    if (I2p_dv)
      _mm_free(I2p_dv);
    if (I2p_du_full)
      _mm_free(I2p_du_full);
    if (I2p_dv_full)
      _mm_free(I2p_dv_full);
    if (m1c1)
      _mm_free(m1c1);
    if (m1c2)
      _mm_free(m1c2);
    ...
    if (I1c_dv)
      _mm_free(I1c_dv);
    if (I1c_du_full)
      _mm_free(I1c_du_full);
    if (I1c_dv_full)
      _mm_free(I1c_dv_full);
    if (m2c1)
      _mm_free(m2c1);
    if (m2c2)
      _mm_free(m2c2);
    ...
    if (I2c_dv)
      _mm_free(I2c_dv);
    if (I2c_du_full)
      _mm_free(I2c_du_full);
    if (I2c_dv_full)
      _mm_free(I2c_dv_full);
  }
  ...
  void Matcher::bucketFeatures(int32_t max_features, float bucket_width, float bucket_height)
  {
    ...
    for (int32_t i = 0; i < bucket_cols * bucket_rows; i++)
    {
      ...
      std::mt19937 g(rd());
      std::shuffle(buckets[i].begin(), buckets[i].end(), g);
  ...
  void Matcher::filterImageAll(uint8_t *I, uint8_t *I_du, uint8_t *I_dv, int16_t *I_f1, int16_t *I_f2, const int *dims)
  {
                             ...
                             result_du++, result_dv++, result_f1++, result_f2++)
    {
      ...
      *result_f1 = -*p00 - *p01 - *p02 - *p03 - *p04 - *p10 + *p11 + *p12 + *p13 - *p14 - *p20 + *p21 + 8 * *p22 + *p23 - *p24 - *p30 + *p31 + *p32 + *p33 - *p34 - *p40 - *p41 - *p42 - *p43 - *p44;
      *result_f2 = -*p00 - *p01 + *p03 + *p04 - *p10 - *p11 + *p13 + *p14 + *p30 + *p31 - *p33 - *p34 + *p40 + *p41 - *p43 - *p44;
    }
  }
      ...
      for (int32_t u_bin = 0; u_bin < u_bin_num; u_bin++)
      {
        ...
        if (delta_accu[v_bin * u_bin_num + u_bin].size() > 0)
        {
               ...
               it != delta_accu[v_bin * u_bin_num + u_bin].end(); it++)
          {
            ...
            {
              if (it->val[i] < delta_min.val[i])
                delta_min.val[i] = it->val[i];
              if (it->val[i] > delta_max.val[i])
